<?php
/**
 * WHMCS SmilePay (速買配) 台灣 全家 FamiPort 超商代碼支付模組 Callback
 *
 * @author      FanYueee(繁月)
...
$gatewayModuleName = 'smilepay_famiport';
...
if ($postData['Classif'] !== 'F' || !isset($postData['Od_sob']) || !isset($postData['Payment_no']) || !isset($postData['Purchamt']) || !isset($postData['Smseid']) || !isset($postData['Mid_smilepay'])) {
    die("0");
}

$calculatedMid = smilepay_famiport_calculateMidSmilepay($gatewayParams['Smseid'], $postData['Purchamt'], $postData['Smseid']);
...
if ($paymentInfo->fami_no !== $postData['Payment_no'] || $paymentInfo->amount != $postData['Purchamt']) {
    die("0");
}
...
function smilepay_famiport_calculateMidSmilepay($smseid, $amount, $callbackSmseid)
{
